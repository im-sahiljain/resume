...
**Basic Format:**
```
https://resume.imsahiljain.in/?source=SOURCE&medium=MEDIUM&campaign=CAMPAIGN&id=USER_ID
```
...
1. **For LinkedIn applications:**
   ```
   https://resume.imsahiljain.in/?source=linkedin&medium=application&campaign=job_search&id=company_abc
   ```

2. **For specific recruiters:**
   ```
   https://resume.imsahiljain.in/?source=email&medium=recruiter&campaign=direct_outreach&id=recruiter_jane_doe
   ```

3. **For networking events:**
   ```
   https://resume.imsahiljain.in/?source=networking&medium=business_card&campaign=tech_conference_2026&id=contact_001
   ```

4. **For job applications:**
   ```
   https://resume.imsahiljain.in/?source=job_portal&medium=application&campaign=senior_sde_roles&id=zeiss_application
   ```
